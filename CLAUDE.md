...
- On narrow screens, tapping a character opens the fixed `.char-popover` dialog instead of scrolling to the sidebar map. The popup shows the character bio, temporarily moves the live relationship map and decoder into the sheet, has a Close button, closes on Esc or backdrop tap, then returns the map and decoder to the sidebar.
