...
- On narrow screens, tapping a character opens the fixed `.char-popover` dialog with the character bio instead of scrolling to the sidebar map. The popup has a Close button, closes on Esc or backdrop tap, and preserves map highlighting in the sidebar for later.
